...
class UploadVideoMixin(object):
    ...
    DELETE_ENDPOINT = '{video_uri}'

    def remove(self, video_uri):
        """Delete the video with the given uri from Vimeo."""
        uri = self.DELETE_ENDPOINT.format(video_uri=video_uri)
        
        temp = self.delete(uri)

        if temp.status_code == 204:
            print 'successfully deleted',video_uri
        elif temp.status_code == 404:
            print 'could not find video',video_uri
        else:
            print 'unknown status',temp.status_code
